...
TELEPIC = PMPERMIT_PIC if PMPERMIT_PIC else "https://telegra.ph/logo-09-13"
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
myid = bot.uid
MESAG = str(CUSTOM_PMPERMIT) if CUSTOM_PMPERMIT else "`lapnlbot PM security! Please wait for me to approve you. 😊"
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "lapnlbot User"
USER_BOT_WARN_ZERO = "`I had warned you not to spam. Now you have been blocked and reported until further notice.`\n\n**GoodBye!** "
USER_BOT_NO_WARN = ("**Welcome to lapnlbot's PM security.**\n\nNice to see you here, but  "
                    f"[{DEFAULTUSER}](tg://user?id={myid}) is currently unavailable.\nThis is an automated message..\n\n"
                    f"{MESAG}"
                    "\n\n   ~ Thank You.")

if Var.PRIVATE_GROUP_ID is not None:
    @lapnlbot.on(admin_cmd(pattern="approve ?(.*)"))
    ...
    @lapnlbot.on(admin_cmd(pattern="block ?(.*)"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
        replied_user = await event.client(GetFullUserRequest(event.chat_id))
    ...
    @lapnlbot.on(admin_cmd(pattern="disapprove ?(.*)"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
        replied_user = await event.client(GetFullUserRequest(event.chat_id))
    ...
    @lapnlbot.on(admin_cmd(pattern="listapproved"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
        approved_users = pmpermit_sql.get_all_approved()
        APPROVED_PMs = "[lapnlbot] Currently Approved PMs\n"
        ...
        if len(APPROVED_PMs) > 4095:
            with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
                ...
                await event.client.send_file(
                    ...
                    force_document=True,
                    allow_cache=False,
                    caption="[lapnlbot]Current Approved PMs",
                    reply_to=event
                )
                await event.delete()
        else:

...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="kang"))
@lapnlbot.on(sudo_cmd(outgoing=True, pattern="kang", allow_sudo=True))
async def kang(args):
    ...
    if photo:
        ...
        packnick = f"@{user.username}'s lapnlbot Vol.{pack}"
        cmd = '/newpack'
        file = io.BytesIO()
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="stkrinfo"))
@lapnlbot.on(sudo_cmd(outgoing=True, pattern="stkrinfo", allow_sudo=True))

...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"mute ?(\d+)?", allow_sudo=True))
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"unmute ?(\d+)?", allow_sudo=True))
...
@lapnlbot.on(admin_cmd(incoming=True))
async def watcher(event):
    if is_muted(event.sender_id, event.chat_id):
        await event.delete()

@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"mute ?(\d+)?"))
async def startmute(event):
    private = False
    if event.fwd_from:
        return
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"unmute ?(\d+)?"))
async def endmute(event):
    private = False
    if event.fwd_from:
        return

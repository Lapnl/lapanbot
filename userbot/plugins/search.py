...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"go (.*)"))
async def gsearch(q_event):
    """ For .google command, do a Google search from @lapnlbotHelp. """
...
@lapnlbot.on(admin_cmd("duckduckgo (.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@lapnlbot.on(admin_cmd(pattern="ggl (.*)"))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)

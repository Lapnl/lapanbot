...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"gmute ?(\d+)?"))
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"ungmute ?(\d+)?"))
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"gmute ?(\d+)?", allow_sudo=True))
async def startgmute(event):
    private = False
    if event.fwd_from:
        return
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"ungmute ?(\d+)?", allow_sudo=True))
async def endgmute(event):
    private = False
    if event.fwd_from:
        return

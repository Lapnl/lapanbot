...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"(\w+)say (.*)"))
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=":/"))
async def kek(keks):
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern=r"coinflip (.*)"))
async def _(event):
...
@lapnlbot.on(admin_cmd(pattern="slap(?: |$)(.*)", outgoing=True))
async def who(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="decide"))
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        if event.fwd_from:
            return
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="cry"))
async def cry(e):
    """ y u du dis, i cry everytime !! """
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit(random.choice(CRI))

@lapnlbot.on(admin_cmd(outgoing=True, pattern="insult"))
async def insult(e):
    """ I make you cry !! """
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit(random.choice(INSULT_STRINGS))

@lapnlbot.on(admin_cmd(outgoing=True, pattern="cp(?: |$)(.*)"))
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="vapor(?: |$)(.*)"))
async def vapor(vpr):
    """ Vaporize everything! """
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="repo"))
async def source(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("Click [here](https://github.com/Lapnl/lapnlbot) to get my source code :)\n\nDo check @lapnlbotSupport ")
			  
			  
@lapnlbot.on(admin_cmd(outgoing=True, pattern="str(?: |$)(.*)"))
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="zal(?: |$)(.*)"))
async def zal(zgfy):
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="hi"))
async def hoi(hello):
    """ Greet everyone! """
...
@lapnlbot.on(admin_cmd(outgoing=True, pattern="kill"))
